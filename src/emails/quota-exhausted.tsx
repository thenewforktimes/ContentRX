...
export function QuotaExhaustedEmail({
  ...
  return (
    <EmailShell preview={`Used all ${quota} ${planLabel} scans for the month.`}>
      <Heading as="h1" style={{ fontSize: 20, marginBottom: 12 }}>
        You've hit this month's limit.
      </Heading>
      <Text>
        All {quota} {planLabel}-plan scans are used. Future scan attempts
        will return an "over quota" error until {resetsAt}.
      </Text>
      {plan === "free" ? (
        <Text>
          Upgrade to Pro (5,000 scans / month) to keep going right now —
          your remaining quota carries over for the rest of the month.
        </Text>
      ) : (
        <Text>
          Your plan resets on the 1st. If you need more scans before then,
