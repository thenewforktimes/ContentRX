...
export const config = {
  matcher: [
    ...
    // their own signatures (api/webhooks, api/cron) or are gated by
    // INTERNAL_EVAL_SECRET (api/evaluate). Skipping clerk middleware on
    // these saves the auth-resolution cost on every webhook delivery —
    // meaningful at Stripe/Clerk volumes.
    "/((?!_next|api/(?:webhooks|cron|evaluate)|[^?]*\\.(?:html?|css|js(?!on)|jpe?g|webp|png|gif|svg|ttf|woff2?|ico|csv|docx?|xlsx?|zip|webmanifest)).*)",
    "/trpc(.*)",
  ],
};
